...
export function OAuthLogin({ onLogin }: OAuthLoginProps) {
  const handleOAuthLogin = (provider: 'google' | 'apple' | 'github') => {
    ...
    // Auto-login after brief delay to simulate OAuth flow
    setTimeout(() => {
      onLogin(provider, mockEmails[provider]);
    }, 300);
  };
      ...
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Transform workout content into device-ready exports
          </p>
        </CardHeader>
        <CardContent className="space-y-3">
          <Button 
            className="w-full gap-2 bg-white text-gray-900 border hover:bg-gray-50" 
          ...
          <div className="relative py-3">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-card px-2 text-muted-foreground">
                Mock OAuth - Auto-login for demo
              </span>
            </div>
          </div>

          <p className="text-xs text-muted-foreground text-center">
            Connect Supabase later for real authentication
          </p>
        </CardContent>
      </Card>
    </div>
